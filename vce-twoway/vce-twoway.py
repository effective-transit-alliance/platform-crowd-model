...
def deboardratefn(k,t,t0,u): # of people on train 1 without ingress. t0 is the train arrival time. train(x)doors*rate goes in u.
    if t >= t0 and k > 0:
        ...
        return 0
def plat_clearance_fn(k,a,w,t,t1): #t is i, t1 is the train 1 arrival time, t2 is the train 2 arrival time.
    if k > 0:
        if (t - t1 < 30):
            return max(min(((111*(a/k)-162)/((a/k)**2))*w/60,19*w/60),0)    #P = (111M - 162)/(M^2) is the upstairs flow eq per ft wide.
        else:
            return max(min(((111*(a/k)-162)/((a/k)**2))*w/60,19*w/60),5*w/60) #Zero clearance rate until train 1 arrives, then hold at a minimum until everyone is upstairs
    else:
        return 0
...
for i in range(0, simtime):
    ...
    if numonplatform < 0:
        numonplatform = 0
    print('time ' + str(i), 'train 1 load ' + str(train1pax), 'train 2 load ' + str(train2pax), train1offrate, train2offrate, numonplatform, instcrowding, plat_egress_rate)
